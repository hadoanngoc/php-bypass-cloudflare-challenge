 ...
 */
 ...
 /**
  * Modified version by hadoanngoc@gmail.com to send and receive cookies 02.03.2019
  **/
...
function curl_get_contents( $url, $return_error_code = false){

    $curl_handle = curl_init();
	
	global $request_cookies, $request_referer;
	$send_cookies = array();
	if(isset($request_cookies) && count($request_cookies) > 0){
		$send_cookies = array_merge($send_cookies, $request_cookies);
	}
	$str = '';
	...
	if($request_referer){
		curl_setopt($curl_handle, CURLOPT_REFERER, $request_referer);
	}
	
	curl_setopt($curl_handle,CURLOPT_USERAGENT,'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36');
	curl_setopt($curl_handle, CURLOPT_HTTPHEADER, array("Cookie: " . $str));
	...
	if(isset($need_cookies_response) && $need_cookies_response){
    ...
    if( !empty( $proxy ) ){
        ...
        if( !empty( $proxy['address'] ) ){
            $address = !empty( $proxy['port'] ) ? "{$proxy['address']}:{$proxy['port']}" : $proxy['address'];
            curl_setopt( $curl_handle, CURLOPT_PROXY, $address );
        }
	...
	curl_setopt($curl_handle, CURLOPT_TIMEOUT, 120); //timeout in seconds
	curl_setopt($curl_handle, CURLOPT_TIMEOUT_MS, 120000); //in miliseconds
    $query = curl_exec( $curl_handle );

    // debug
    if( $errno = curl_errno( $curl_handle ) ) {

        $error_message = curl_strerror( $errno );

        $message = "cURL error ({$errno}): {$error_message}. URL : {$url}.";
        $message = !empty( $proxy ) && is_array( $proxy ) ? $message . 'Proxy : ' . json_encode( $proxy ) : '';

        return $errno;
    }
	
	preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $query, $matches);
...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    ...
    $contents = curl_get_contents($url, $use_include_path, $context, $offset);
